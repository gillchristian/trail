...
<wpt lat="46.0072" lon="7.0123">
  ...
  <desc>Km 22.6 · Water, Food · Rest 5:00</desc>
  <sym>Restaurant</sym>
...
- `<name>` — the station's name, emitted **verbatim**. The exporter has no `Aid 1`/`Aid 2` fallback of its own — whatever name the station carries is what's written (an unnamed station yields an empty `<name>`).
- `<desc>` — generated by `buildDesc` (`src/GpxExport.elm`): `Km <dist> · <services, comma-joined> · Rest <m:ss>`, dropping any empty part (no services → no service segment; no rest → no `Rest` segment).
- `<sym>` — Garmin-standard symbol, **auto-derived from the station's services** by `symbolForAid` (`src/GpxExport.elm:112`): Food → `Restaurant`, else Water → `Drinking Water`, else Medical → `First Aid`, else → **`Flag, Blue`** (the no-services default). It is **not** user-selectable — there is no per-station `<sym>` picker in the UI.

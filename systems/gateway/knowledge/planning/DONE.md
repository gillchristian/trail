...
- TASK-006 — Broaden OAuth scope to `profile:read_all` — 2026-05-15 — one-line scope addition in `StravaRedirect` + migration 017 invalidates the SummaryAthlete-shape cache rows (`DELETE FROM activity_cache WHERE activity_id < 0`). Trail addendum 1. PR #10 (merge sha in git log). See journal 2026-05-15 19:30.

...
- ~~**TASK-024 — Strava OAuth integration in trail.**~~ **v1 shipped (PR #25).** Token storage + settings-page Connect/Disconnect. Activity picker + streams fetch split off as **TASK-024b** (pending — needs `elm/http`, an activity-picker modal, and the streams→splits wiring via `StravaStreams` + the existing `ActualGpx.computeSplits`).

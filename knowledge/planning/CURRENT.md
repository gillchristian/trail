...
1. **Slice 1 — merge state on `Race` + `.trail` (headless, inert):** move `PlanningLayer` `Merge`→`Types` (the `ChangeEntry` import-cycle fix again), add `Race.mergeBase : Maybe PlanningLayer` + `Race.version : Dict String Int` (codecs, ride `raceMetaFields`, `D.oneOf` defaults); bump version in `commitRaceEdit`; set `mergeBase`/`version` at export (share point). Extend `smoke:merge`/`smoke:trailsync` (version bump, base round-trip). **No import-merge routing yet** → inert + back-compat, like the WI-5 data slices.
2. **Slice 2 — entry point + classify + apply + the review modal (browser-verified):** route a matching-`shareId` import to `classifyVersions`; FastForward auto-applies; Diverged opens the modal; the apply path + `Merged` feed entries; the hand-merge custom-value set. Held for the user's in-browser check, like the feed/flows.

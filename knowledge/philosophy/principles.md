...
4. **Reversibility before speed.** Every change must be revertible. Commit often (one logical change per commit). Never overwrite uncommitted work I didn't create.
...
9. **Don't break the existing frontend.** Cadence has a live frontend (`client/`) and a single real user. Every backend change must keep the existing flows working. Verify with the cadence client locally before declaring done.
